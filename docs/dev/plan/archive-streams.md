...
At completion, equivalent content produces identical public byte streams and offset arrays across plain, ZIP, TAR, and 7z inputs. Archive validation, terminal integrity, early close, and resource cleanup obey one public contract despite backend differences. Encrypted or password-protected archives are outside the supported input domain.
...
Document any immutable corrupt fixture that cannot be generated reliably. Do not commit large archives or call production code to construct expected inputs.
...
### Milestone: TAR streaming

#### Task: implement streaming TAR
...
### Milestone: archive lifecycle certification
...
5. Run corrupt, truncated, multi-member, and special-entry cases.
